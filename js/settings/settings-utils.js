...
(function(ns){
    ...
    ns.setLoading = (btn, isLoading, text) => { if (btn) { btn.disabled = isLoading; btn.textContent = text; } };
    ns.showError = (el, message) => { if (el) { el.textContent = message; el.style.display = 'block'; } };
    ns.hideError = (el) => { if (el) el.style.display = 'none'; };
})(window.SettingsApp);
